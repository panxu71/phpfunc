...
class Str
{
     ...
     * @return string
     */
    public static function randomString($length = 6, $isupper = true, $islower = false, $isspec = false)
    {
    ...
    public static function uuid(bool $connector = true, bool $mark = true, bool $lower = false): string
    {
        if (function_exists('com_create_guid')) {
            ...
            !$connector && $uuid = str_replace("-", "", $uuid);
            !$mark && $uuid      = trim($uuid, '{}');
            $lower && $uuid      = strtoupper($uuid);
            return $uuid;
        }
        $connector               = $connector ? "-" : "";
        $mark                    = $mark ? ["{", "}"] : ["", ""];
        ...
        return !$lower ? strtolower($uuid) : $uuid;
    }
     ...
     * @param  string  $issurname  固定姓
     * @param  boolen  $iscompound 是否加入复姓 true 是 false 否
     * @return array   ['surname' => '姓', 'name' => '名', 'username' => '姓名']
     */
    public static function randomName($sex = 0, $issurname = null, $iscompound = true)
    {
        // 单姓(常见姓氏508个)
        $sigleSurname = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤', '许', '何', '吕', '施', '张', '孔', '曹', '严', '华', '金', '魏', '陶', '姜', '戚', '谢', '邹', '喻', '柏', '水', '窦', '章', '云', '苏', '潘', '葛', '奚', '范', '彭', '郎', '鲁', '韦', '昌', '马', '苗', '凤', '花', '方', '俞', '任', '袁', '柳', '酆', '鲍', '史', '唐', '费', '廉', '岑', '薛', '雷', '贺', '倪', '汤', '滕', '殷', '罗', '毕', '郝', '邬', '安', '常', '乐', '于', '时', '傅', '皮', '卞', '齐', '康', '伍', '余', '元', '卜', '顾', '孟', '平', '黄', '和', '穆', '萧', '尹', '姚', '邵', '湛', '汪', '祁', '毛', '禹', '狄', '米', '贝', '明', '臧', '计', '伏', '成', '戴', '谈', '宋', '茅', '庞', '熊', '纪', '舒', '屈', '项', '祝', '董', '梁', '杜', '阮', '蓝', '闵', '席', '季', '麻', '强', '贾', '路', '娄', '危', '江', '童', '颜', '郭', '梅', '盛', '林', '刁', '钟', '徐', '邱', '骆', '高', '夏', '蔡', '田', '樊', '胡', '凌', '霍', '虞', '万', '支', '柯', '昝', '管', '卢', '莫', '经', '房', '裘', '缪', '干', '解', '应', '宗', '丁', '宣', '贲', '邓', '郁', '单', '杭', '洪', '包', '诸', '左', '石', '崔', '吉', '钮', '龚', '程', '嵇', '邢', '滑', '裴', '陆', '荣', '翁', '荀', '羊', '於', '惠', '甄', '麴', '家', '封', '芮', '羿', '储', '靳', '汲', '邴', '糜', '松', '井', '段', '富', '巫', '乌', '焦', '巴', '弓', '牧', '隗', '山', '谷', '车', '侯', '宓', '蓬', '全', '郗', '班', '仰', '秋', '仲', '伊', '宫', '宁', '仇', '栾', '暴', '甘', '钭', '厉', '戎', '祖', '武', '符', '刘', '景', '詹', '束', '龙', '叶', '幸', '司', '韶', '郜', '黎', '蓟', '薄', '印', '宿', '白', '怀', '蒲', '邰', '从', '鄂', '索', '咸', '籍', '赖', '卓', '蔺', '屠', '蒙', '池', '乔', '阴', '欎', '胥', '能', '苍', '双', '闻', '莘', '党', '翟', '谭', '贡', '劳', '逄', '姬', '申', '扶', '堵', '冉', '宰', '郦', '雍', '舄', '璩', '桑', '桂', '濮', '牛', '寿', '通', '边', '扈', '燕', '冀', '郏', '浦', '尚', '农', '温', '别', '庄', '晏', '柴', '瞿', '阎', '充', '慕', '连', '茹', '习', '宦', '艾', '鱼', '容', '向', '古', '易', '慎', '戈', '廖', '庾', '终', '暨', '居', '衡', '步', '都', '耿', '满', '弘', '匡', '国', '文', '寇', '广', '禄', '阙', '东', '殴', '殳', '沃', '利', '蔚', '越', '夔', '隆', '师', '巩', '厍', '聂', '晁', '勾', '敖', '融', '冷', '訾', '辛', '阚', '那', '简', '饶', '空', '曾', '毋', '沙', '乜', '养', '鞠', '须', '丰', '巢', '关', '蒯', '相', '查', '後', '荆', '红', '游', '竺', '权', '逯', '盖', '益', '桓', '公', '万俟', '司马', '上官', '欧阳', '夏侯', '诸葛', '闻人', '东方', '赫连', '皇甫', '尉迟', '公羊', '澹台', '公冶', '宗政', '濮阳', '淳于', '单于', '太叔', '申屠', '公孙', '仲孙', '轩辕', '令狐', '钟离', '宇文', '长孙', '慕容', '鲜于', '闾丘', '司徒', '司空', '亓官', '司寇', '仉', '督', '子车', '颛孙', '端木', '巫马', '公西', '漆雕', '乐正', '壤驷', '公良', '拓跋', '夹谷', '宰父', '谷梁', '晋', '楚', '闫', '法', '汝', '鄢', '涂', '钦', '段干', '百里', '东郭', '南门', '呼延', '归', '海', '羊舌', '微生', '岳', '帅', '缑', '亢', '况', '后', '有', '琴', '梁丘', '左丘', '东门', '西门', '商', '牟', '佘', '佴', '伯', '赏', '南宫', '墨', '哈', '谯', '笪', '年', '爱', '阳', '佟', '第五', '言', '福', '百', '家', '姓', '终'];
        // 复姓
        $compoundSurname = ['司马', '上官', '欧阳', '夏侯', '诸葛', '闻人', '东方', '赫连', '皇甫', '尉迟', '公羊', '澹台', '公冶', '宗政', '濮阳', '淳于', '单于', '太叔', '申屠', '公孙', '仲孙', '轩辕', '令狐', '徐离', '宇文', '长孙', '慕容', '司徒', '司空'];

        // 男性名
        $manName = ['伟', '刚', '勇', '毅', '俊', '峰', '强', '军', '平', '保', '东', '文', '辉', '力', '明', '永', '健', '世', '广', '志', '义', '兴', '良', '海', '山', '仁', '波', '宁', '贵', '福', '生', '龙', '元', '全', '国', '胜', '学', '祥', '才', '发', '武', '新', '利', '清', '飞', '彬', '富', '顺', '信', '子', '杰', '涛', '昌', '成', '康', '星', '光', '天', '达', '安', '岩', '中', '茂', '进', '林', '有', '坚', '和', '彪', '博', '诚', '先', '敬', '震', '振', '壮', '会', '思', '群', '豪', '心', '邦', '承', '乐', '绍', '功', '松', '善', '厚', '庆', '磊', '民', '友', '裕', '河', '哲', '江', '超', '浩', '亮', '政', '谦', '亨', '奇', '固', '之', '轮', '翰', '朗', '伯', '宏', '言', '若', '鸣', '朋', '斌', '梁', '栋', '维', '启', '克', '伦', '翔', '旭', '鹏', '泽', '晨', '辰', '士', '以', '建', '家', '致', '树', '炎', '德', '行', '时', '泰', '盛', '雄', '琛', '钧', '冠', '策', '腾', '楠', '榕', '风', '航', '弘', '乐', '宏', '言', '旲', '旻', '昊', '宁', '贵', '福', '生', '龙', '光', '天', '达', '安', '岩', '中', '茂', '进', '林', '有', '坚', '和', '彪', '博', '泰', '盛', '振', '哲', '江', '超', '炎', '德', '明', '永', '健', '世', '广', '志', '义', '兴', '良', '海', '山', '仁', '波', '宁', '行', '时', '志', '忠', '思', '彰', '征', '律', '绍', '功', '松', '善', '厚', '庆', '磊', '民', '友', '裕', '河', '晨', '辰', '士', '以', '建', '家', '致', '煜', '煊', '炎', '波', '胜', '学', '祥', '才', '发', '武', '新', '利', '清', '飞', '彬', '富', '顺', '信', '子', '杰', '涛', '昌', '成', '元', '全', '国', '康', '星', '光', '天', '达', '安', '岩', '中', '茂', '进', '林', '有', '坚', '和', '彪', '博', '诚', '先', '敬', '绍', '功', '松', '震', '振', '壮', '会', '思', '群', '豪', '心', '邦', '承', '乐', '友', '裕', '河', '哲', '江', '超', '浩', '亮', '政', '谦', '亨', '言', '若', '鸣', '朋', '斌', '梁', '栋', '维', '启', '克', '伦', '翔', '旭', '鹏', '泽', '晨', '辰', '士', '以', '民', '奇', '固', '建', '家', '致', '树', '炎', '德', '行', '时', '泰', '盛', '雄', '琛', '钧', '冠', '策', '腾', '楠', '榕', '善', '厚', '庆', '磊', '之', '轮', '翰', '朗', '伯', '宏'];

        // 女性名
        $womanName = ['秀', '娟', '英', '华', '慧', '巧', '美', '娜', '静', '淑', '惠', '珠', '翠', '雅', '芝', '玉', '萍', '红', '娥', '玲', '芬', '芳', '燕', '彩', '春', '菊', '兰', '凤', '洁', '梅', '琳', '素', '云', '莲', '真', '环', '雪', '荣', '爱', '妹', '霞', '香', '月', '莺', '媛', '艳', '瑞', '凡', '佳', '嘉', '琼', '勤', '珍', '贞', '莉', '桂', '娣', '叶', '璧', '璐', '娅', '琦', '晶', '妍', '茜', '秋', '珊', '莎', '锦', '黛', '青', '倩', '婷', '姣', '婉', '娴', '瑾', '颖', '露', '瑶', '怡', '婵', '雁', '蓓', '纨', '仪', '荷', '丹', '蓉', '眉', '君', '琴', '蕊', '薇', '菁', '梦', '岚', '苑', '婕', '馨', '瑗', '琰', '韵', '融', '园', '艺', '咏', '卿', '聪', '澜', '纯', '毓', '悦', '昭', '冰', '爽', '琬', '茗', '羽', '希', '欣', '飘', '育', '滢', '馥', '筠', '柔', '竹', '霭', '凝', '晓', '欢', '霄', '枫', '芸', '菲', '寒', '伊', '亚', '宜', '可', '姬', '舒', '影', '荔', '枝', '丽', '阳', '妮', '宝', '贝', '初', '程', '梵', '罡', '恒', '鸿', '桦', '骅', '剑', '娇', '纪', '宽', '苛', '灵', '玛', '媚', '琪', '晴', '容', '睿', '烁', '堂', '唯', '威', '韦', '雯', '苇', '萱', '阅', '彦', '宇', '雨', '洋', '忠', '宗', '曼', '紫', '逸', '贤', '蝶', '菡', '绿', '蓝', '儿', '翠', '烟', '媛', '瑾', '嘉', '春', '璐', '飘', '月', '卿', '妍', '希', '娣', '颖', '莺', '艳', '慧', '倩', '雁', '惠', '兰', '薇', '婉', '真', '叶', '雅', '淑', '昭', '洁', '茗', '琴', '荣', '翠', '菊', '蕊', '蓓', '娴', '苑', '萍', '妹', '悦', '燕', '菁', '馨', '香', '园', '环', '瑞', '锦', '爽', '珍', '融', '云', '莎', '咏', '璧', '霞', '娅', '露', '欣', '梦', '羽', '滢', '韵', '婵', '眉', '澜', '冰', '纯', '君', '娜', '凡', '聪', '彩', '馥', '红', '晶', '琼', '育', '琦', '怡', '玉', '岚', '琳', '素', '梅', '琰', '芝', '芳', '纨', '秋', '巧', '桂', '瑗', '娥', '丹', '莉', '毓', '雪', '珊', '贞', '琬', '佳', '静', '芬', '荷', '黛', '宁', '青', '姣', '勤', '艺', '婕', '瑶', '茜', '仪', '凤', '蓉', '玲', '莲', '爱', '美'];

        // 获取姓
        if (!$issurname) {
            $surname   = $iscompound ? array_merge($sigleSurname, $compoundSurname) : $sigleSurname;
            $issurname = $surname[mt_rand(0, count($surname) - 1)];
        }
        // 获取名
        switch ($sex) {
            case 1:
            ...
            case 2:
                ...
                break;
            default:
                $name  = array_merge($manName, $womanName);
                break;
        }

        // 单字名
        $uname = $name[mt_rand(0, count($name) - 1)];
        // 双字名
        mt_rand(0, 1) && $uname .= $name[mt_rand(0, count($name) - 1)];
        return ['surname' => $issurname, 'name' => $uname, 'username' => $issurname . $uname];
    }

    /**
     * 随机邮箱
     * @param  string $type 指定类型邮箱
     * @param  string $len  指定邮箱长度(不包括@后面的长度)
     * @return string       3l619ad@qq.com
     */
    public static function randomEmail($type = "", $len = 6)
    {
        $emails = [
            '126'     => "@126.com",
            '163'     => "@163.com",
        ...
        return self::randomString($len) . $emails[$type ? $type : array_rand($emails)];
    }
     ...
     */
    public static function randomDate($begin = "", $end = "", $now = true)
    {
    ...
    /**
     * 生成随机手机号码
     * @param  integer $operator 运营商类型(默认随机;0,移动;1,联通;2,电信)
     * @return string            13244859784
     */
    public static function randomPhone($operator = null)
    {
        $paragraph = [
     ...
     * @param  integer $num 小数位
     * @return float/string 1.23
     */
    public static function randomFloat($num = 2, $min = 0, $max = 10)
    {
     ...
     *
     * @param boolean $isIndex 是否返回省份下标
     * @return array
     */
    public static function randomProvince(bool $isIndex = false, bool $isAll = false): array
    {
        $province = [
            "北京市", "天津市", "河北省", "山西省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "上海市", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "湖北省", "湖南省", "广东省", "广西壮族自治区", "海南省", "重庆市", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "青海省", "宁夏回族自治区", "新疆维吾尔自治区", "香港特别行政区", "澳门特别行政区", "台湾省"
        ];
        $index                     = array_rand($province, 1);
        $isIndex && $data['index'] = $index;
     ...
     * 返回随机颜色
     *
     * @param integer $num 颜色数量，默认返回一种
     * @param array $columns 返回颜色字段，默认返回十六进制（0英文名|1中文名|2十六进制|3RGB）
     * @return void
     */
    public static function randomColor(int $num = 2, array $columns = [2]): array
    {
        $colorList = [
        ...
        $indexs = (array)array_rand($colorList, $num);
        $data = [];
        foreach ($indexs as $k => $v) {
            if (!isset($colorList[$v])) continue;
            if (!count($columns)) {
                $data[$k] = $colorList[$v];
            } else {
                foreach ($columns as $key => $column) {
                    isset($colorList[$v][$column]) && $data[$k][$key] = $colorList[$v][$column];
                }
            }
        }
        return $data;
     ...
     * @param string $operation 操作类型 ENCODE加密 DECODE 解密
     * @param string $key       密钥，用于加密和解密
     * @param integer $expiry   有效期，用于加密操作，单位为秒，默认值为 0，即永久有效
     * @return string           返回加密或解密后的字符串
     */
    public static function encryption(string $string = "", string $operation = 'DECODE', string $key = '', int $expiry = 0): string
    {
        $ckey_length = 4;
        $key = md5($key ? $key : 'https://panxu.net');
